...
test.describe("ASN & Network Metadata End-to-End Tests", () => {
  test("should lookup pure ASN with prefix (AS15169) and render entity, RIR, and subnets", async ({ page }) => {
    ...
    // Enter AS15169
    const searchInput = page.locator("input[placeholder*='15169']");
  ...
  test("should lookup pure ASN with raw digits (15169) and properly resolve ASN metadata", async ({ page }) => {
    await page.goto("/registration/asn");

    const searchInput = page.locator("input[placeholder*='15169']");
    await searchInput.fill("15169");
    await searchInput.press("Enter");
