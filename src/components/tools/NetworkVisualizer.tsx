...
export const NetworkVisualizer: React.FC = () => {
    ...
    while (queue.length > 0) {
      ...
      if (currentId === endId) {
        ...
        for (let i = 0; i < path.length - 1; i++) {
          ...
          // Firewall / Router blocking check
  ...
  const showDropAnimation = (nodeId: string) => {
    ...
    if (!node || !packetLayerRef.current) return;

    const g = document.createElementNS("http://www.w3.org/2000/svg", "g");
    const xMark = document.createElementNS("http://www.w3.org/2000/svg", "text");
    xMark.textContent = "×";
    xMark.setAttribute("x", node.x.toString());
    xMark.setAttribute("y", (node.y + 12).toString());
    xMark.setAttribute("text-anchor", "middle");
    xMark.setAttribute("font-size", "48");
    xMark.setAttribute("fill", "#ef4444");
    xMark.setAttribute("font-weight", "bold");
    xMark.setAttribute("style", "pointer-events: none;");
    g.appendChild(xMark);
    packetLayerRef.current.appendChild(g);
    ...
    logEvent(`Packet DROPPED at ${node.name}`, 'error');
  };
  ...
  const animatePacket = (path: string[], color: string = '#fbbf24', callback?: () => void, failureId?: string) => {
    if (path.length < 2 || !packetLayerRef.current) {
      if (failureId && path.length === 1 && path[0] === failureId) {
        showDropAnimation(failureId);
      } else {
        callback?.();
      }
      return;
    ...
    animate(packetCircle, {
      ...
      onComplete: () => {
        ...
        animatePacket(path.slice(1), color, callback, failureId);
      }
    });
  };
  ...
  return (
    <div className="flex flex-col h-full space-y-4 p-4">
      <div className="flex flex-wrap items-center justify-between gap-3 bg-card p-3 rounded-lg border shadow-sm">        <div className="flex flex-wrap items-center gap-3">
                  ...
                  <div className="p-3 bg-blue-50/50 dark:bg-blue-900/10 rounded-lg border border-blue-100 dark:border-blue-900/30 space-y-3">
                    <div className="flex items-center justify-between">
                      ...
                      <Button 
                        ...
                        onClick={() => {
                          const otherPcs = nodes.filter(n => n.id !== selectedNode && (n.type === DeviceType.PC || n.type === DeviceType.SERVER));
                          otherPcs.forEach(target => handlePing(selectedNode!, target.id));
                        }}
                        title="Broadcast from this device"
                      >
                    ...
                    <div className="space-y-1.5">
                      ...
                      <Select value={pingTargetId} onValueChange={setPingTargetId}>
                        ...
                        <SelectContent>
                          ...
                          {nodes.filter(n => n.id !== selectedNode && (n.type === DeviceType.PC || n.type === DeviceType.SERVER)).length === 0 && (
                            <div className="p-2 text-[10px] text-muted-foreground italic">No other targets found</div>
                          )}
                        </SelectContent>
                      </Select>
                    </div>
